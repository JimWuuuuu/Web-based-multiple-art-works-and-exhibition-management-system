...
include_once('db_conn.php');
		$sql_art = "SELECT * FROM `artist` ORDER BY `artist_id` DESC";
		$sql_a = mysqli_query($conn, $sql_art);
...
<div class="row">
<?php
	if ($sql_a && $sql_a->num_rows > 0) 
	{
		while ($r = $sql_a->fetch_assoc()) 
		{
			
			$rid=$r['artist_id'];
			$sql_b = "SELECT * FROM `artist` where artist_id='$rid'";
...
?>	
	<div class="card text-center" style="width: 35rem;">
		...
		<?php echo '<img src="data:image/png;base64,'.base64_encode($r2['artist_pic']).'">';?>
		<div class="card-body">
			<h5 class="card-title"><font size='6'><?php echo $r2['artist_name']?></font></h5>
		</div>
		<div class="card-body">
		<?php if( $r2['artist_link_web'] != "")
			{
		?>
			<img src="internet.png" height="30px"><a href="<?php echo $r2['artist_link_web']?>" class="card-link"><font size='4'>個人網站</font></a></img><p><p>
		<?php	
			}
		?>
		<?php if( $r2['artist_link_ig'] != "")
			{
		?>
			<img src="instagram.png" height="30px"><a href="<?php echo $r2['artist_link_ig']?>" class="card-link"><font size='4'>Instagram</font></a></img><p><p>
		<?php	
			}
		?>
		<?php if( $r2['artist_link_fb'] != "")
			{
		?>
			<img src="facebook.png" height="30px"><a href="<?php echo $r2['artist_link_fb']?>" class="card-link"><font size='4'>Facebook</font></a></img><p><p>
		<?php	
			}
		?>
		<?php if( $r2['artist_link_twi'] != "")
			{
		?>
			<img src="twitter.png" height="30px"><a href="<?php echo $r2['artist_link_twi']?>" class="card-link"><font size='4'>Twitter</font></a></img><p><p>
		<?php	
			}
		?>
		...
		<?php
			if(isset($_SESSION['user_name']) && $_SESSION['level']>=1){
		?>  
		<img src="delete.png" height="30px"><a href='delete_art_php.php?artist_id=<?php echo $r2['artist_id']; ?>' class="card-link"><font size='4'>刪除藝術家</font></a></img><p><p>
